    ...
    */
    ...
    function goEnsureDecisionMakerZone(){
      ...
      zone.innerHTML=`<div class="dm-premium-nav" id="dm-premium-nav"><button type="button" data-dm-jump="courses">COURSES</button><button type="button" data-dm-jump="challenges">CHALLENGES</button><button type="button" data-dm-jump="tracker">TRACKER</button><button type="button" data-dm-jump="onthego">ON THE GO</button><button type="button" data-dm-jump="sessions">SESSIONS</button><button type="button" data-dm-jump="community">COMMUNITY</button></div><div class="go-zone-head"><div><small>FREE 30 DAY CHALLENGES</small><h2>MAKE THE NEXT 30 DAYS COUNT</h2><p>Choose one challenge at a time. Your progress, levels, achievements and history stay connected to your B.O.S.S CODE GO account.</p></div><button id="go-account-button" class="go-account-button" type="button">${goProfile?'MY PROFILE':'SIGN IN / CREATE PROFILE'}</button></div><div id="go-challenge-grid" class="go-challenge-grid"><div class="go-empty" style="grid-column:1/-1">LOADING CHALLENGES...</div></div><div class="dm-tracker-heading" id="dm-tracker-anchor"><small>YOUR CURRENT MOVE</small><h2>ACTIVE TRACKER</h2></div><div id="go-active-strip" class="go-active-strip"></div>`;
    ...
    function goEnsureDecisionMakerCommunitySections(){
      ...
      if(!$('dm-community-home')){
        ...
        community.innerHTML='<div class="decision-heading"><div><span class="decision-kicker">MOVE TOGETHER</span><h2>DECISION MAKERS COMMUNITY</h2></div><span class="red-line"></span></div><p class="decision-section-copy">See milestones, progress and approved inspiration from the Decision Makers community.</p><div id="dm-community-preview" class="dm-community-preview"><div class="go-empty">SIGN IN TO SEE COMMUNITY ACTIVITY.</div></div><button id="dm-open-community" class="dm-community-open" type="button">OPEN ACTIVITY FEED</button>';
    ...
    async function goLoadCommunityPreview(){
      ...
      if(!goToken){box.innerHTML='<div class="go-empty">SIGN IN TO SEE COMMUNITY ACTIVITY.</div>';return;}
      try{const data=await goFetch('/go/community?limit=4');const items=Array.isArray(data.activity)?data.activity:[];box.innerHTML=items.length?items.map(a=>`<article class="dm-community-card"><div class="go-feed-head">${a.profile_photo_url?`<img class="go-avatar" src="${esc(a.profile_photo_url)}" alt="">`:'<div class="go-avatar"></div>'}<div><strong>${esc(a.display_name||'B.O.S.S CODE GO Member')}</strong><small>${esc(a.activity_type||'PROGRESS')}</small></div></div><h3>${esc(a.title||'')}</h3><p>${esc(a.body||'')}</p></article>`).join(''):'<div class="go-empty">COMMUNITY ACTIVITY WILL APPEAR HERE.</div>';}
      catch(e){box.innerHTML='<div class="go-empty">SIGN IN TO SEE COMMUNITY ACTIVITY.</div>';}
    }
